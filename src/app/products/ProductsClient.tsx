...
export default function ProductsClient({ initialSearch }: { initialSearch?: string }) {
  ...
  useEffect(() => {
    setActiveFilters(prev => ({...prev, searchTerm: search}));
  }, [search]);
  ...
  // New Arrivals Badge (safe check)
  const badge = product.badge || '';
  const newArrivalsMatch = activeFilters.newArrivals ? badge.toLowerCase().includes('new') : true;
      ...
      <div className="grid lg:grid-cols-[280px_1fr] gap-8">
        <aside className="hidden lg:block">
            <ProductFilters 
              ...
              initialFilters={{...initialFilters, searchTerm: search}}
            />
        </aside>
